...
import net.ee.pfanalyzer.IDebugInformation;
...
public class TestIO {
	...
	public static void main(String[] args) {
		CaseData cs = new CaseData();
		ModelDBData db = new ModelDBData();
		db.getModelClass().add(createBusData());
...
//		p1.set
		...
		m1.setModelNumber(BigInteger.valueOf(1));
		c1.getModel().add(m1);
		db.getModelClass().add(c1);
		cs.setModelDb(db);
		try {
			CaseSerializer serializer = new CaseSerializer();
			serializer.writeCase(cs, new File(IDebugInformation.MODEL_DB_FILE));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
